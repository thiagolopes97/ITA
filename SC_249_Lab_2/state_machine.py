...
class WayPoint(State):
    ...
    def check_transition(self, agent, state_machine):
        ...
        '''
           Agent -> Classe Roomba/ Drone
           State Machine -> Estado da FSM que se encontra ou no caso, para realizar a mudanca
        '''
    ...
    def execute(self, agent, target = None):
        ...
        '''
            Atribuicao da missao e criacao dos pontos targets
        '''
...
class PaparazziEight(State):
    ...
    def execute(self, agent):
        ...
        if agent.drone_target == None:
            agent.drone_target = (3,2,
                                  4,4)

        p1 = agent.drone_target[:2]
        p2 = agent.drone_target[2:]
        x_1, y_1, x_2, y_2 = agent.drone_target

        # Criacao dos pontos auxiliares
        ...
        # Atribuindo o ponto P2 (Do relatório)
        ...
        if self.etapa == 'Approach First':
            ...
            if dist(focal_point, (x_c, y_c)) <= 0.04:
                self.etapa = 'Right Curve'


        elif self.etapa == 'Right Curve':
...
class StayAt(State):
    ...
    def check_transition(self, agent, state_machine):
        ...
        if (self.tempo > 20) | (self.movimento == 2):
    ...
    def execute(self, agent):
        ...
        if self.etapa == 'Approach First':
            ...
            if (deg < 1) & (deg > 0):
                ...
                agent.set_velocity(0.5,  self.sentido / radius)
                self.deg = deg
